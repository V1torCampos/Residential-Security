...
// Definição dos pinos do sensor e alarme
...
byte pinosLinhas[LINHAS] = {23, 22, 3, 21}; 
byte pinosColunas[COLUNAS] = {19, 18, 5, 17}; 

Keypad Customkeypad = Keypad(makeKeymap(teclas), pinosLinhas, pinosColunas, LINHAS, COLUNAS);

// Declaração do semáforo
SemaphoreHandle_t TarefaSensorMovimento;
SemaphoreHandle_t TarefaSenha;
...
void setup()
{
  ...
  Blynk.virtualWrite(V2, SenhaTeste);
	...
	// Criação do semáforo (1 indica que é um semáforo binário)
	TarefaSensorMovimento = xSemaphoreCreateBinary();
	TarefaSenha = xSemaphoreCreateBinary();
      ...
      case HIGH:
        ...
        digitalWrite(LED_STATUS, HIGH);
          ...
          if (digitalRead(SENSOR_MOVIMENTO_PINO) == HIGH || digitalRead(SENSOR_ABERTURA_PINO) == LOW )
          {
              ...
              Serial.println(SENSOR_ABERTURA_PINO);

							///////////// atualiza a hora e imprime no blynk
							timeClient.update();
		...
		vTaskDelay(pdMS_TO_TICKS(1000));	// Intervalo de verificação do sensor de movimento
	}
}
...
void tarefaSenha(void *pvParameters)
{
	while (1)
	{
      ...
      switch (ATIVADO) {
      case LOW:
        Serial.println("Tarefas habilitadas!");
        ATIVADO = HIGH;
        digitalWrite(LED_STATUS, HIGH);
        break;
      
      case HIGH:
        Serial.println("Tarefas desabilitadas!");
        ATIVADO = LOW;
        digitalWrite(LED_STATUS, LOW);
        break;
      } 
		...
		// se a senha digitada estiver incorreta, ativa o alarme
		if (senhaDigitada != "123") {

        Serial.println("Senha Inorreta");
        SenhaTeste = senhaDigitada;
